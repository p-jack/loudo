...
describe("Stash", () => {
  ...
  test("sameSeq", () => {
    const ds1 = stash([1, 2, 3, 4, 5])
    expect(ds1.sameSeq(ds1)).toStrictEqual(true)
    expect(ds1.sameSeq([1, 2, 3, 4, 5])).toStrictEqual(true)
    expect(ds1.sameSeq([1, 2, 3, 4])).toStrictEqual(false)
    const ds2 = stash([1, 2, 3, 4, 5])
    expect(ds1.sameSeq(ds2)).toStrictEqual(true)
    const ds3 = stash([1, 2, 10000, 4, 5])
    expect(ds1.sameSeq(ds3)).toStrictEqual(false)
    const s1 = sized([1, 2, 3, 4, 5], () => 5)
    const s2 = sized([1, 2, 3, 4], () => 4)
    expect(s1.sameSeq(s2)).toStrictEqual(false)
  })
  test("toJSON", () => {
    const ds = stash([1, 2, 3])
